...
function Sidebar({
  ...
  useEffect(() => {
    const handleFieldEngineerUpdate = (udpateFE: FieldEngineer) => {
      console.log("🔄 Field Engineer update received:", udpateFE);

      if (selectedFE && selectedFE.id === udpateFE.id) {
        setSelectedFE(udpateFE);
      }
    };
    subscribe("ReceiveFieldEngineerUpdate", handleFieldEngineerUpdate);

    return () => {
      unsubscribe("ReceiveFieldEngineerUpdate", handleFieldEngineerUpdate);
    };
  }, [selectedFE]);
  ...
  return (
    <aside
  className={`fixed right-0 top-0 h-full bg-[#6b6f1d]/95 backdrop-blur-3xl border-l border-white/10 transition-all duration-300 z-40 flex flex-col shadow-2xl ${
...
>
      ...
      <div className="p-2 border-b border-white/10">
        ...
        >
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className={`w-5 h-5 text-white transition-transform duration-300 ${sidebarCollapsed ? "rotate-180" : ""
              }`}
          >
            <path
              strokeLinecap="round"
      ...
      {/* Tab Navigation */}
      <div
        className={`p-2 border-b border-white/10 transition-all duration-300 ${sidebarCollapsed ? "opacity-0 pointer-events-none" : "opacity-100"
          }`}
      >
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 p-2 rounded-lg transition-colors ${activeTab === "engineers"
          ...
          >
            ...
            <span className="text-xs font-medium">Field Engineers</span>
      ...
      {/* Expanded Content */}
      <div
        className={`flex-1 overflow-hidden transition-all duration-300 ${sidebarCollapsed ? "opacity-0 pointer-events-none" : "opacity-100"
          }`}
      >
        {/* Search Bar */}
        ...
        {/* ✅ Status Filter */}
        <div className="p-3 mt-3 bg-white/5 rounded-lg border border-white/10 flex flex-wrap gap-3 text-xs text-white">
        ...
        <aside
          className={` right-0 top-63 h-full max-h-screen bg-[#6b6f1d]/95 backdrop-blur transition-all duration-300 z-40 flex flex-col shadow-2xl ${sidebarCollapsed ? "w-12" : "w-80"
            }`}
        >
          {/* Content Area */}
